...
struct Loc {
    ...
    public:
        ...
        inline friend bool operator==(const Loc& a, const Loc& b) {
...
namespace std {
  template <>
  struct hash<Loc > {
    inline size_t operator()(const Loc& p) const {
      return p.x * 1812433253 + p.y;
    }
  };
  template <>
  struct hash<Box > {
    inline size_t operator()(const Box& b) const {
      return b.x * 1812433253 + b.y;
    }
  };
  template <>
  struct hash<Bomb > {
    inline size_t operator()(const Bomb& b) const {
      return b.x * 1812433253 + b.y;
    }
  };
}
...
struct Grid {
  ...
  inline bool inGrid(Loc p) const {
    return 0 <= p.x && p.x < width && 0 <= p.y && p.y < height;
  }

  inline bool isBox(Loc id) const {
    return boxes.count(id);
  }

  inline bool isWall(Loc id) const {
    return walls.count(id);
  }

  inline bool isBomb(Loc id) const {
    return bombs.count(id);
  }

  inline bool isItem(Loc id) const {
...
inline double heuristic(Loc a, Loc b) {
...
double dist(const Loc& p1, const Loc& p2) {
...
void computeBestMoves(map<double, Loc>& bestLoc) {
    ...
    for (auto move : playerPath) {
        ...
        //cerr<<"for possible "<<printLoc(bombLoc)<<endl;
        int nbBoxBlowed;
        ...
        if (nbBoxBlowed) { // valid candidate
            ...
            if (safeBomb) {
                ...
                if (safePos) {
                    ...
                    double load = nbBoxBlowed - dist/8;
...
void itemResolutionAndMapUpdate(int height, int myId) {
    ...
    for (auto mapItem : grid.items) {
        ...
        if (!stillHere) { toRemove.push_back(mapItem.first); }
        }
    }
...
bool isNextSecure(Loc p) {
    ...
    for (auto bomb : bombs) {
        ...
        for (auto n : next) {
          if (!bombRanges[bomb].count(n)) {nextHasEscape = true; break;}
        }
...
Loc findSecureNextLoc (string& msg) {
      ...
      for (auto bomb : bombs) {
          ...
          loc.x = -1; loc.y = -1;
          nextLoc.x = player.x; nextLoc.y = player.y;
          while (count > 1) {
            loc = nextLoc;
            nextLoc = potPath[loc];
            count--;
        ...
        if (grid.boxes.size() > 0) {
            ...
            if (bestLoc.size()) {
                ...
                for (map<double, Loc>::reverse_iterator it = bestLoc.rbegin(); it != bestLoc.rend(); ++it) {
                    ...
                    for (auto bomb : bombs) {
                        cerr <<bomb<<endl;
                        if((target.x == bomb.x && abs(target.y-bomb.y) <= bomb.blow()) || (target.y == bomb.y && abs(target.x-bomb.x) <= bomb.blow())) {
                            cerr << "is too close, not secure"<<endl;
                            secure = false;
                            break;
                        }
                    }
            ...
            Loc en(0, 0);
            for (auto e : ennemies) {
                if (dist(player, e) < close) {
                    close = static_cast<int>(dist(player, e));
                    en = e;
                }
            }
